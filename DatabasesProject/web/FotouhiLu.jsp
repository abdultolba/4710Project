...
<content id="content">
    <h1>Papers which Fotouhi and Lu have co-authored</h1>
    <form>
        ...
        <%
            try {
                ...
                ResultSet rs;
                rs = stmt.executeQuery("SELECT * FROM paper WHERE paperid IN (SELECT paperid FROM written WHERE email = (SELECT email FROM author WHERE name = 'Lu')) AND paperid IN (SELECT paperid FROM written WHERE email=(SELECT email FROM author WHERE name = 'Fotouhi'));");
                while ( rs.next() ) { %>
